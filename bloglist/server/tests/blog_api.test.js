...
describe('when there is initially some blogs saved', () => {
  ...
  describe('deletion of a blog', () => {
    ...
    test('fails with status 401 if user is not the owner', async () => {
      ...
      assert(blogsAtEnd.some((blog) => blog.id === response.body.id))
    })
  ...
  describe('updating a blog', () => {
    ...
    test('the updated blog is saved in the database', async () => {
      ...
      const updatedInMongo = blogsAtEnd.find((blog) => blog.id === toUpdate.id)

      assert.strictEqual(updatedInMongo.likes, toUpdate.likes + 1)
    })
  })
})
